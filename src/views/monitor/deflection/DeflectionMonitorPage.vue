<template>
  <div class="deflection-page">
    <div class="page-header">
      <div>
        <h2 class="page-title">挠度监测</h2>
        <p class="page-subtitle">计算结果类挠度数据监控</p>
      </div>
      <div class="page-badge">DeflectionMonitorPage</div>
    </div>
    ...
    <section class="section-card">
      <div class="section-header">
        <h3 class="section-title">最新挠度结果</h3>
      ...
      <div v-if="latestData" class="data-grid">
        ...
        <div class="data-item">
          <div class="data-label">挠度值</div>
          <div class="data-value">{{ formatDisplayValue(latestData.deflectionValue) }}</div>
        </div>
        <div class="data-item">
          <div class="data-label">核验状态</div>
          <div class="data-value">{{ formatVerifyStatus(latestData.verifyStatus) }}</div>
        </div>
    ...
    <section class="section-card">
      <div class="section-header">
        ...
        <span class="section-tag">deflection</span>
      </div>
      ...
      <div v-if="chartPoints.length > 0" class="chart-box">
        <svg class="trend-chart" viewBox="0 0 1000 320" preserveAspectRatio="none">
          ...
          <text x="20" y="20" class="chart-title-text">挠度值</text>
    ...
    <section class="section-card">
      <div class="section-header">
        <h3 class="section-title">历史挠度明细</h3>
      ...
      <div v-if="historyList.length > 0" class="table-wrapper">
        <table class="history-table">
          ...
          <tbody>
            ...
            >
              <td>{{ item.sensorId || "-" }}</td>
              <td>{{ formatDisplayValue(item.deflectionValue) }}</td>
              <td>{{ formatVerifyStatus(item.verifyStatus) }}</td>
...
import { postDeflectionHistory, postDeflectionLatest } from "@/api/deflection";

defineOptions({
  name: "DeflectionMonitorPage",
});
...
function formatVerifyStatus(value) {
  if (value === 0 || value === "0") return "仅原始";
  if (value === 1 || value === "1") return "待核验";
  if (value === 2 || value === "2") return "核验通过";
  if (value === 3 || value === "3") return "核验驳回";
  return "-";
}
...
async function handleQuery() {
  ...
  try {
    ...
    const latestResponse = await postDeflectionLatest({
      sensorId,
      limit: 1,
    });
    ...
    const historyResponse = await postDeflectionHistory({
      sensorId,
    ...
    latestData.value = null;
    historyList.value = [];
    errorMessage.value = error?.response?.data?.message || error?.message || "挠度模块查询失败";
  } finally {
    loading.value = false;
  }
}
...
const chartSeries = computed(() => {
  return historyList.value
    .map((item, index) => {
      const rawValue = item?.deflectionValue;
