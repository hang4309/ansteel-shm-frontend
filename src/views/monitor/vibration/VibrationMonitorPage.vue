<template>
  <div class="vibration-page">
    <div class="page-header">
      <div>
        <h2 class="page-title">振动监测</h2>
        <p class="page-subtitle">传感器直采振动数据实时监控</p>
      </div>
      <div class="page-badge">VibrationMonitorPage</div>
    </div>
    ...
    <section class="section-card">
      <div class="section-header">
        <h3 class="section-title">最新振动数据</h3>
      ...
      <div v-if="latestData" class="data-grid">
        ...
        <div class="data-item">
          ...
          <div class="data-value">{{ formatDisplayValue(latestData.acceleration) }}</div>
        </div>
        <div class="data-item">
          <div class="data-label">频率</div>
          <div class="data-value">{{ formatDisplayValue(latestData.frequency) }}</div>
        </div>
        <div class="data-item">
          <div class="data-label">振幅</div>
          <div class="data-value">{{ formatDisplayValue(latestData.amplitude) }}</div>
        </div>
      ...
      <div class="section-header">
        ...
        <div class="metric-switch">
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'acceleration' }"
            @click="chartMetric = 'acceleration'"
          >
            加速度
          </button>
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'frequency' }"
            @click="chartMetric = 'frequency'"
          >
            频率
          </button>
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'amplitude' }"
            @click="chartMetric = 'amplitude'"
          >
            振幅
          </button>
        </div>
    ...
    <section class="section-card">
      <div class="section-header">
        <h3 class="section-title">历史振动明细</h3>
      ...
      <div v-if="historyList.length > 0" class="table-wrapper">
        <table class="history-table">
          ...
          <tbody>
            ...
            >
              <td>{{ item.sensorId || "-" }}</td>
              <td>{{ formatDisplayValue(item.acceleration) }}</td>
              <td>{{ formatDisplayValue(item.frequency) }}</td>
              <td>{{ formatDisplayValue(item.amplitude) }}</td>
...
import { getVibrationHistory, getVibrationLatest } from "@/api/vibration";

defineOptions({
  name: "VibrationMonitorPage",
});
...
const chartMetric = ref("acceleration");
...
const chartMetricLabel = computed(() => {
  if (chartMetric.value === "acceleration") {
    return "加速度";
  }

  if (chartMetric.value === "frequency") {
    return "频率";
  }
...
async function handleQuery() {
  ...
  try {
    ...
    const latestResponse = await getVibrationLatest({
      sensorId,
    });

    latestData.value = normalizeObject(unwrapResultData(latestResponse));

    const historyResponse = await getVibrationHistory({
      sensorId,
    ...
    latestData.value = null;
    historyList.value = [];
    errorMessage.value = error?.response?.data?.message || error?.message || "振动模块查询失败";
  } finally {
    loading.value = false;
  }
}
