<template>
  <div class="safety-page">
    <div class="page-header">
      <div>
        <h2 class="page-title">安全监测</h2>
        <p class="page-subtitle">计算结果类安全状态数据实时监控</p>
      </div>
      <div class="page-badge">SafetyMonitorPage</div>
    </div>
    ...
    <section class="section-card">
      <div class="section-header">
        <h3 class="section-title">最新安全数据</h3>
      ...
      <div v-if="latestData" class="data-grid">
        ...
        <div class="data-item">
          <div class="data-label">安全等级</div>
          <div class="data-value">{{ formatDisplayValue(latestData.safetyLevel) }}</div>
        </div>
        <div class="data-item">
          <div class="data-label">预警标记</div>
          <div class="data-value">{{ formatDisplayValue(latestData.warningFlag) }}</div>
        </div>
        <div class="data-item">
          <div class="data-label">损伤趋势</div>
          <div class="data-value">{{ formatDisplayValue(latestData.damageTrend) }}</div>
        </div>
      ...
      <div class="section-header">
        ...
        <div class="metric-switch">
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'safetyLevel' }"
            @click="chartMetric = 'safetyLevel'"
          >
            安全等级
          </button>
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'warningFlag' }"
            @click="chartMetric = 'warningFlag'"
          >
            预警标记
          </button>
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'damageTrend' }"
            @click="chartMetric = 'damageTrend'"
          >
            损伤趋势
          </button>
        </div>
    ...
    <section class="section-card">
      <div class="section-header">
        <h3 class="section-title">历史安全明细</h3>
      ...
      <div v-if="historyList.length > 0" class="table-wrapper">
        <table class="history-table">
          ...
          <tbody>
            ...
            >
              <td>{{ item.sensorId || "-" }}</td>
              <td>{{ formatDisplayValue(item.safetyLevel) }}</td>
              <td>{{ formatDisplayValue(item.warningFlag) }}</td>
              <td>{{ formatDisplayValue(item.damageTrend) }}</td>
...
import { getSafetyHistory, getSafetyLatest } from "@/api/safety";

defineOptions({
  name: "SafetyMonitorPage",
});
...
const chartMetric = ref("safetyLevel");
...
const chartMetricLabel = computed(() => {
  if (chartMetric.value === "safetyLevel") {
    return "安全等级";
  }

  if (chartMetric.value === "warningFlag") {
    return "预警标记";
  }
...
async function handleQuery() {
  ...
  try {
    ...
    const latestResponse = await getSafetyLatest({
      sensorId,
    });

    latestData.value = normalizeObject(unwrapResultData(latestResponse));

    const historyResponse = await getSafetyHistory({
      sensorId,
    ...
    latestData.value = null;
    historyList.value = [];
    errorMessage.value = error?.response?.data?.message || error?.message || "安全模块查询失败";
  } finally {
    loading.value = false;
  }
}
