<template>
  <div class="strain-page">
    <div class="page-header">
      <div>
        <h2 class="page-title">应变监测</h2>
        <p class="page-subtitle">传感器直采应变数据实时监控</p>
      </div>
      <div class="page-badge">StrainMonitorPage</div>
    </div>
    ...
    <section class="section-card">
      <div class="section-header">
        <h3 class="section-title">最新应变数据</h3>
      ...
      <div v-if="latestData" class="data-grid">
        ...
        <div class="data-item">
          <div class="data-label">微应变</div>
          <div class="data-value">{{ formatDisplayValue(latestData.microStrain) }}</div>
        </div>
        <div class="data-item">
          <div class="data-label">应变分布</div>
          <div class="data-value">{{ formatDisplayValue(latestData.strainDistribution) }}</div>
        </div>
        <div class="data-item">
          <div class="data-label">关键点应变</div>
          <div class="data-value">{{ formatDisplayValue(latestData.keyPointStrain) }}</div>
        </div>
      ...
      <div class="section-header">
        ...
        <div class="metric-switch">
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'microStrain' }"
            @click="chartMetric = 'microStrain'"
          >
            微应变
          </button>
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'strainDistribution' }"
            @click="chartMetric = 'strainDistribution'"
          >
            应变分布
          </button>
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'keyPointStrain' }"
            @click="chartMetric = 'keyPointStrain'"
          >
            关键点应变
          </button>
        </div>
    ...
    <section class="section-card">
      <div class="section-header">
        <h3 class="section-title">历史应变明细</h3>
      ...
      <div v-if="historyList.length > 0" class="table-wrapper">
        <table class="history-table">
          ...
          <tbody>
            ...
            >
              <td>{{ item.sensorId || "-" }}</td>
              <td>{{ formatDisplayValue(item.microStrain) }}</td>
              <td>{{ formatDisplayValue(item.strainDistribution) }}</td>
              <td>{{ formatDisplayValue(item.keyPointStrain) }}</td>
...
import { getStrainHistory, getStrainLatest } from "@/api/strain";

defineOptions({
  name: "StrainMonitorPage",
});
...
const chartMetric = ref("microStrain");
...
const chartMetricLabel = computed(() => {
  if (chartMetric.value === "microStrain") {
    return "微应变";
  }

  if (chartMetric.value === "strainDistribution") {
    return "应变分布";
  }
...
async function handleQuery() {
  ...
  try {
    ...
    const latestResponse = await getStrainLatest({
      sensorId,
    });

    latestData.value = normalizeObject(unwrapResultData(latestResponse));

    const historyResponse = await getStrainHistory({
      sensorId,
    ...
    latestData.value = null;
    historyList.value = [];
    errorMessage.value = error?.response?.data?.message || error?.message || "应变模块查询失败";
  } finally {
    loading.value = false;
  }
}
