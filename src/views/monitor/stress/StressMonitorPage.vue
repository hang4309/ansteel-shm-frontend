<template>
  <div class="stress-page">
    <div class="page-header">
      <div>
        <h2 class="page-title">应力监测</h2>
        <p class="page-subtitle">应力结果数据实时监控</p>
      </div>
      <div class="page-badge">StressMonitorPage</div>
    </div>
    ...
    <section class="section-card">
      <div class="section-header">
        <h3 class="section-title">最新应力数据</h3>
      ...
      <div v-if="latestData" class="data-grid">
        ...
        <div class="data-item">
          <div class="data-label">主应力</div>
          <div class="data-value">{{ formatDisplayValue(latestData.principalStress) }}</div>
        </div>
        <div class="data-item">
          <div class="data-label">等效应力</div>
          <div class="data-value">{{ formatDisplayValue(latestData.equivalentStress) }}</div>
        </div>
        <div class="data-item">
          <div class="data-label">应力幅值</div>
          <div class="data-value">{{ formatDisplayValue(latestData.stressAmplitude) }}</div>
        </div>
        <div class="data-item">
          <div class="data-label">热点应力</div>
          <div class="data-value">{{ formatDisplayValue(latestData.hotspotStress) }}</div>
        </div>
    ...
    <section class="section-card">
      <div class="section-header">
        ...
        <div class="metric-switch">
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'principalStress' }"
            @click="chartMetric = 'principalStress'"
          >
            主应力
          </button>
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'equivalentStress' }"
            @click="chartMetric = 'equivalentStress'"
          >
            等效应力
          </button>
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'stressAmplitude' }"
            @click="chartMetric = 'stressAmplitude'"
          >
            应力幅值
          </button>
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'hotspotStress' }"
            @click="chartMetric = 'hotspotStress'"
          >
            热点应力
          </button>
        </div>
      ...
      <div v-if="chartPoints.length > 0" class="chart-box">
        <svg class="trend-chart" viewBox="0 0 1000 320" preserveAspectRatio="none">
          ...
          <text x="20" y="20" class="chart-title-text">{{ chartMetricLabel }}</text>
          <text x="952" y="308" class="chart-title-text" text-anchor="end">采集时间</text>
        </svg>

        <div class="chart-meta">
          <span>当前指标：{{ chartMetricLabel }}</span>
    ...
    <section class="section-card">
      <div class="section-header">
        <h3 class="section-title">历史应力明细</h3>
      ...
      <div v-if="historyList.length > 0" class="table-wrapper">
        <table class="history-table">
          ...
          <tbody>
            ...
            >
              <td>{{ item.sensorId || "-" }}</td>
              <td>{{ formatDisplayValue(item.principalStress) }}</td>
              <td>{{ formatDisplayValue(item.equivalentStress) }}</td>
              <td>{{ formatDisplayValue(item.stressAmplitude) }}</td>
              <td>{{ formatDisplayValue(item.hotspotStress) }}</td>
...
import { getStressHistory, getStressLatest } from "@/api/stress";

defineOptions({
  name: "StressMonitorPage",
});
...
const chartMetric = ref("principalStress");
...
const chartMetricLabel = computed(() => {
  if (chartMetric.value === "principalStress") {
    return "主应力";
  }
  if (chartMetric.value === "equivalentStress") {
    return "等效应力";
  }
  if (chartMetric.value === "stressAmplitude") {
    return "应力幅值";
  }
  return "热点应力";
});
...
async function handleQuery() {
  ...
  try {
    ...
    const latestResponse = await getStressLatest({
      sensorId,
    });

    latestData.value = normalizeObject(unwrapResultData(latestResponse));

    const historyResponse = await getStressHistory({
      sensorId,
      startTime: form.startTime,
      endTime: form.endTime,
      page: 1,
      size: Number(form.limit),
    });
    ...
    latestData.value = null;
    historyList.value = [];
    errorMessage.value = error?.response?.data?.message || error?.message || "应力模块查询失败";
  } finally {
    loading.value = false;
  }
}
...
const chartSeries = computed(() => {
  return historyList.value
    .map((item, index) => {
      const rawValue = item?.[chartMetric.value];
