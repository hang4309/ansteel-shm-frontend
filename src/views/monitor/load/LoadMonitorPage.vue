<template>
  <div class="load-page">
    <div class="page-header">
      <div>
        <h2 class="page-title">荷载监测</h2>
        <p class="page-subtitle">传感器直采荷载数据实时监控</p>
      </div>
      <div class="page-badge">LoadMonitorPage</div>
    </div>
    ...
    <section class="section-card">
      <div class="section-header">
        <h3 class="section-title">最新荷载数据</h3>
      ...
      <div v-if="latestData" class="data-grid">
        ...
        <div class="data-item">
          <div class="data-label">起吊重量</div>
          <div class="data-value">{{ formatDisplayValue(latestData.liftingWeight) }}</div>
        </div>
        <div class="data-item">
          <div class="data-label">运行速度</div>
          <div class="data-value">{{ formatDisplayValue(latestData.travelSpeed) }}</div>
        </div>
        <div class="data-item">
          <div class="data-label">冲击系数</div>
          <div class="data-value">{{ formatDisplayValue(latestData.impactFactor) }}</div>
        </div>
      ...
      <div class="section-header">
        ...
        <div class="metric-switch">
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'liftingWeight' }"
            @click="chartMetric = 'liftingWeight'"
          >
            起吊重量
          </button>
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'travelSpeed' }"
            @click="chartMetric = 'travelSpeed'"
          >
            运行速度
          </button>
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'impactFactor' }"
            @click="chartMetric = 'impactFactor'"
          >
            冲击系数
          </button>
        </div>
    ...
    <section class="section-card">
      <div class="section-header">
        <h3 class="section-title">历史荷载明细</h3>
      ...
      <div v-if="historyList.length > 0" class="table-wrapper">
        <table class="history-table">
          ...
          <tbody>
            ...
            >
              <td>{{ item.sensorId || "-" }}</td>
              <td>{{ formatDisplayValue(item.liftingWeight) }}</td>
              <td>{{ formatDisplayValue(item.travelSpeed) }}</td>
              <td>{{ formatDisplayValue(item.impactFactor) }}</td>
...
import { getLoadHistory, getLoadLatest } from "@/api/load";

defineOptions({
  name: "LoadMonitorPage",
});
...
const chartMetric = ref("liftingWeight");
...
const chartMetricLabel = computed(() => {
  if (chartMetric.value === "liftingWeight") {
    return "起吊重量";
  }

  if (chartMetric.value === "travelSpeed") {
    return "运行速度";
  }
...
async function handleQuery() {
  ...
  try {
    ...
    const latestResponse = await getLoadLatest({
      sensorId,
    });

    latestData.value = normalizeObject(unwrapResultData(latestResponse));

    const historyResponse = await getLoadHistory({
      sensorId,
    ...
    latestData.value = null;
    historyList.value = [];
    errorMessage.value = error?.response?.data?.message || error?.message || "荷载模块查询失败";
  } finally {
    loading.value = false;
  }
}
