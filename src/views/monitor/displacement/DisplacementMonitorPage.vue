<template>
  <div class="displacement-page">
    <div class="page-header">
      <div>
        <h2 class="page-title">位移监测</h2>
        <p class="page-subtitle">传感器直采位移数据实时监控</p>
      </div>
      <div class="page-badge">DisplacementMonitorPage</div>
    </div>
    ...
    <section class="section-card">
      <div class="section-header">
        <h3 class="section-title">最新位移数据</h3>
      ...
      <div v-if="latestData" class="data-grid">
        ...
        <div class="data-item">
          <div class="data-label">垂向挠度</div>
          <div class="data-value">{{ formatDisplayValue(latestData.verticalDeflection) }}</div>
        </div>
        <div class="data-item">
          <div class="data-label">水平位移</div>
          <div class="data-value">{{ formatDisplayValue(latestData.horizontalDisplacement) }}</div>
        </div>
        <div class="data-item">
          <div class="data-label">跨中位移</div>
          <div class="data-value">{{ formatDisplayValue(latestData.midspanDisplacement) }}</div>
        </div>
      ...
      <div class="section-header">
        ...
        <div class="metric-switch">
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'verticalDeflection' }"
            @click="chartMetric = 'verticalDeflection'"
          >
            垂向挠度
          </button>
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'horizontalDisplacement' }"
            @click="chartMetric = 'horizontalDisplacement'"
          >
            水平位移
          </button>
          <button
            type="button"
            class="metric-btn"
            :class="{ active: chartMetric === 'midspanDisplacement' }"
            @click="chartMetric = 'midspanDisplacement'"
          >
            跨中位移
          </button>
        </div>
    ...
    <section class="section-card">
      <div class="section-header">
        <h3 class="section-title">历史位移明细</h3>
      ...
      <div v-if="historyList.length > 0" class="table-wrapper">
        <table class="history-table">
          ...
          <tbody>
            ...
            >
              <td>{{ item.sensorId || "-" }}</td>
              <td>{{ formatDisplayValue(item.verticalDeflection) }}</td>
              <td>{{ formatDisplayValue(item.horizontalDisplacement) }}</td>
              <td>{{ formatDisplayValue(item.midspanDisplacement) }}</td>
...
import { getDisplacementHistory, getDisplacementLatest } from "@/api/displacement";

defineOptions({
  name: "DisplacementMonitorPage",
});
...
const chartMetric = ref("verticalDeflection");
...
const chartMetricLabel = computed(() => {
  if (chartMetric.value === "verticalDeflection") {
    return "垂向挠度";
  }

  if (chartMetric.value === "horizontalDisplacement") {
    return "水平位移";
  }
...
async function handleQuery() {
  ...
  try {
    ...
    const latestResponse = await getDisplacementLatest({
      sensorId,
    });

    latestData.value = normalizeObject(unwrapResultData(latestResponse));

    const historyResponse = await getDisplacementHistory({
      sensorId,
    ...
    latestData.value = null;
    historyList.value = [];
    errorMessage.value = error?.response?.data?.message || error?.message || "位移模块查询失败";
  } finally {
    loading.value = false;
  }
}
